...
const subjectsPath = "singleLearnPath/subjects";

/** Same pattern as `components/subjects/exam/SubjectExamView` */
export default function ScientificTrackSubjectExamView() {
  ...
  const ex = translate?.pages.scientificTrackSubjects.subjectExam;
  ...
  const { data: exam, isLoading, isError, error } =
      ...
      <div className="max-w-4xl mx-auto py-16 px-4 text-center space-y-6">
        ...
        <div className="flex flex-wrap justify-center gap-3">
          <Button
            type="button"
            onClick={() =>
              router.push(`/${lang}/${subjectsPath}/exam/${subjectId}/edit`)
            }
          >
            {ex?.createExamBtn}
          </Button>
        ...
        <CardContent className="px-4 py-8 md:px-10 md:py-10 space-y-8">
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <Button
              type="button"
              variant="outline"
              onClick={() => router.push(`/${lang}/${subjectsPath}`)}
            >
              {ex?.backSubjects ?? pg?.viewSubject.backBtn}
            </Button>
          </div>
        </CardContent>
      </Card>
